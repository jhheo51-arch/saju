...
test("서버가 직접 사주를 계산하고 Gemini에는 원본 날짜·시각을 빼고 보낸다", async () => {
  ...
  await withGeminiMock(async (_url, init) => {
    ...
    assert.match(outbound.system_instruction, /신비한 예언자나 교과서처럼 말하지 말고.*오래 대화한 상담자처럼/);
    assert.match(outbound.system_instruction, /속으로만.*이 사주의 중심축.*서로 밀고 당기는 두 면.*반복될 생활 장면.*도움이 될 선택 기준/);
    assert.match(outbound.system_instruction, /meaning은 단서들이 만나 만드는 중심 흐름을 2~3문장/);
    assert.match(outbound.system_instruction, /scene은 실제로 겪을 법한 한 장면을 2문장/);
    assert.match(outbound.system_instruction, /balance는 그 강점이 지나칠 때의 반대 모습을 1~2문장/);
    assert.match(outbound.system_instruction, /같은 풀이를 다른 사주나 다른 관심 주제에 이름만 바꿔 쓸 수 있다면/);
    assert.match(outbound.system_instruction, /장점에는 그것이 과해졌을 때의 불편한 모습도 반드시 짝지으세요/);
